...
function determineExportables(record) {
    ...
    if (record.sdsHazardStatements !== 'Hazard Statements not found or are not identified for this SDS' && record.sdsHazardStatements !== 'No valid hazard statements found or are not identified for this SDS') {
...
function formatCellValue(value) {
    if (Array.isArray(value)) {
        ...
        if (value.length && typeof value[0] === 'object' && value[0] !== null) {
            ...
            value.forEach((v, i) => {
                // Underlined message
                sections.push({ text: v.message + '\n', font: { underline: true } });

                // SDS line
                sections.push({ text: 'SDS: ' });
                sections.push({ text: (v.sdsValue || '') + '\n', font: { bold: true } });

                // PubChem line
                sections.push({ text: 'PubChem: ' });
                sections.push({ text: (v.pubChemValue || '') + '\n', font: { bold: true } });

                // Penalty line
                sections.push({ text: 'Penalty: ' });
                sections.push({ text: (v.penalty != null ? String(v.penalty) : ''), font: { bold: true } });
            ...
            const cleaned = sections.filter(s => s && typeof s.text === 'string' && s.text.trim() !== '');
            return { richText: cleaned };
        }

        // Handle array of strings/numbers
        // If all entries are short codes (2-letter values), join with commas
        const isShortCodes = value.every(v => typeof v === 'string' && /^[A-Za-z]{2}$/.test(v.trim()));
        if (isShortCodes) {
...
function addBasicFormating(sheet) { 
    ...
    sheet.columns.forEach((col, index) => {
        ...
        if ([2, 4, 5, 7, 9, 11, 12].includes(columnNumber)) {
            ...
            col.width = Math.min(Math.max(calculatedWidth, MIN_COL_WIDTH), MAX_COL_WIDTH);
        }
    });
}
...
function generateSummary(record) {
  ...
  richRuns.push(
    ...
    const cleaned = richRuns.filter(s => s && typeof s.text === 'string' && s.text.trim() !== '');
    return { richText: cleaned };
...
function generateSuccessReport(successReport) {
    ...
    richRuns.push(
        ...
        { text: 'Runtime:\n' },
        { text: String(successReport.runTime), font: { bold: true, underline: true } },
        { text: '\n_______________________________\n' },
