...
export async function scrapeFisherSDS(chemicalData, page, cookieString) {
	...
	const searchUrl = `https://www.fishersci.com/us/en/catalog/search/sds?selectLang=EN&store=&msdsKeyword=${encodeURIComponent(
		searchQuery,
	)}`;
	...
	for (let i = 0; i < sdsLinks.length; i++) {
		...
		if (
			!bestSDS.sdsConfidenceScore ||
			(sdsData.sdsConfidenceScore &&
				sdsData.sdsConfidenceScore > bestSDS.sdsConfidenceScore)
		) {
...
async function pdfExtract(currLink, cookieString) {
	try {
		...
		let cleanText = cleanUpText(rawText);

		cleanText = splitBySections(cleanText);
