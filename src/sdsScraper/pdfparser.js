...
export function pdfParse(chemicalData, textPath) {
    ...
    try {
        ...
        if (revision && revision[1]) {
            ...
            if (dateMatch) {
                ...
                if (!isNaN(parsedDate.getTime())) {
                    ...
                    sdsData.errorCode.push('Could not check Revision Date Automatically');
        ...
        if (productName && productName[1]) {
            ...
            sdsData.errorCode.push("Error: could not retrieve product name from SDS");
        }
        ...
        //Remove page headers - Fisher has page breaks with name/revision date headers, we want to filter out all of these to not get parsing issues.
            ...
            .replace(new RegExp(escapeRegex(sdsData.sdsProductName), 'gi'), '')
            .replace(/Revision Date/gi, '')
            .replace(new RegExp(escapeRegex(sdsData.sdsRevisionDate), 'gi'), '');
        ...
        if (casNumber && casNumber[1]) {
            ...
            sdsData.errorCode.push("Error: could not retrieve CAS number from SDS");
        }
        ...
        if (synonyms && synonyms[1]) {
            ...
            sdsData.errorCode.push("Error: could not retrieve synonyms from SDS");
        }

        //Grab Signal Word
        const signalWord = cleanedText.match(/Signal Word[:\s]*([\s\S]*?)(?=Hazard)/i);
        if (signalWord && signalWord[1]) {
            sdsData.sdsSignalWord = signalWord[1].trim();
        }
        else {
            sdsData.sdsSignalWord = 'Signal word not found';
            sdsData.errorCode.push("Error: could not retrieve Signal Word from SDS");
        }
        ...
        const storageNotes = storageSection.match(/[Ss]torage\s*[.:–-]*\s*([\s\S]*)/i);
        ...
        if (molecularFormula && molecularFormula[1]) {
            sdsData.sdsMolecularFormula = molecularFormula[1].trim().replace(/\s+/g, '');
        } else {
            sdsData.sdsMolecularFormula = 'Molecular Formula not found';
            sdsData.errorCode.push("Error: could not retrieve molecular formula from SDS");
        }
        ...
        if (molecularWeight && molecularWeight[1]) {
            ...
            sdsData.errorCode.push("Error: could not retrieve molecular weight from SDS");
        }
        ...
        console.log('classification:', classification);
            if (classification) {
                ...
                sdsData.errorCode.push('Error: could not classify chemical via name/formula')
            }
        ...
        sdsData.errorCode.push(`ERROR parsing ALL sds information. ${err.message}`)
    }
    return sdsData;
  }
...
function validateData(chemicalData, sdsData) {
    ...
    if (sdsData.sdsCASNumber !== chemicalData.searchQuery) {
        ...
        const matchInPubChem = chemicalData.pubChemCASNumbers.includes(sdsData.sdsCASNumber);

        if (matchInPubChem) {
            confidenceScore -= 20;
            console.log(`SDS CAS (${sdsData.sdsCASNumber}) matches a PubChem CAS number (${chemicalData.pubChemCASNumbers}) but not searchQuery(${chemicalData.searchQuery}), -20`, confidenceScore);
            confidenceScoreInfo.push(`SDS CAS (${sdsData.sdsCASNumber}) matches a PubChem CAS number (${chemicalData.pubChemCASNumbers}) but not searchQuery(${chemicalData.searchQuery}), -20`)
            ...
            confidenceScore -= 40;
            console.log(`SDS CAS (${sdsData.sdsCASNumber}) does not match PubChem CAS numbers (${chemicalData.pubChemCASNumbers}) or searchQuery (${chemicalData.searchQuery}), -40`, confidenceScore);
            confidenceScoreInfo.push({
    ...
    //check Name (filtered)
    const normalizedSDSName = normalizeChemicalName(sdsData.sdsProductName || '');
    const normalizedPubChemName = normalizeChemicalName(chemicalData.pubChemProductName || '');
    ...
    } else if (normalizedSDSName.includes(normalizedPubChemName) || normalizedPubChemName.includes(normalizedSDSName)) {
    ...
    //check signal word
    if (chemicalData.pubChemSignalWord.toLowerCase().trim() === sdsData.sdsSignalWord.toLowerCase().trim()) {
        console.log(`Signal words match, SDS: ${sdsData.sdsSignalWord}, PubChem: ${chemicalData.pubChemSignalWord} -0`, `Confidence Score: ${confidenceScore}`);
    } else {
        confidenceScore -= 5;
        console.log(`Signal words do not match, SDS: ${sdsData.sdsSignalWord}, PubChem: ${chemicalData.pubChemSignalWord} -5`, `Confidence Score: ${confidenceScore}`);
        confidenceScoreInfo.push({
    ...
    const normalizedSDSFormula = normalizeMolecularFormula(sdsData.sdsMolecularFormula);
    ...
    if (sdsNormalizedDate && sdsNormalizedDate > cutoffDate) {
        console.log(`SDS revision date (${sdsNormalizedDate.toISOString().split('T')[0]}) is after cutoff: (${cutoffDate.toISOString().split('T')[0]}), -0`, `Confidence Score: ${confidenceScore}`)

    } else {
        confidenceScore -= 40;
        console.log(`SDS revision date (${sdsNormalizedDate.toISOString().split('T')[0]}) is BEFORE cutoff: (${cutoffDate.toISOString().split('T')[0]}), -40`, `Confidence Score: ${confidenceScore}`)
        confidenceScoreInfo.push(`SDS revision date (${sdsNormalizedDate.toISOString().split('T')[0]}) is BEFORE cutoff: (${cutoffDate.toISOString().split('T')[0]}), -40`);
        confidenceScoreInfo.push({
...
function generateClassification(chemicalData, sdsData) {
    let n = (sdsData.sdsProductName || chemicalData.pubChemProductName || '').toLowerCase().trim(); //name
    let f = (
    sdsData.sdsMolecularFormula !== "Molecular Formula not found"
        ? sdsData.sdsMolecularFormula
        : chemicalData.pubChemMolecularFormula || ''
    ).toUpperCase().trim(); //formula
    ...
    const classificationRules = [
        ...
        { 
            test: (n, f) => f.includes('C') && f.includes('H') && n.includes('acid'), 
            result: 'Organic acid' 
        },
        {
        test: (n, f) =>
        ...
        {
            test: (n, f) =>
                /(nitrate|sulfate|chloride|carbonate|phosphate|bromide|iodide|fluoride|acetate|permanganate|chromate)/i.test(n) ||
