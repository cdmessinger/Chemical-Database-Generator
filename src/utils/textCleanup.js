...
export function cleanHazardBlock(text) {
  ...
  .replace(/\+?\d[\d\s\-()]{6,}\d/g, "")
  .replace(/(\b(?:Causes|May(?:\s+cause|\s+form)?|Harmful|Fatal|Toxic|Flammable|H\d{3})\b|\()/gi, "\n$1")  // insert line break first
  ...
  .trim();
    ...
    const HAZARD_STARTERS = /^(H\d{3}|Causes|May(?:\s+cause|\s+form)?|Harmful|Fatal|Toxic|Flammable)\b/i;
  ...
  .map(line => {
    ...
    return L
      .replace(/\bAcute(\s+oral)?\s+toxicity\b/gi, "")
      ...
      .replace(/\s*\(?\s*Combustible\s+dust\s*[:\-–—]?\s*Yes\s*\)?\s*$/i, "")
...
const filteredWords = [
  'Company', 'Fisher Scientific', 'One Reagent', 'Inc.', '30 Bond Street', 'Ward Hill',
  'MA 01835-8099', 'Tel:', '800-343-0660', 'Fax:', '800-322-4757', 'Emergency Telephone Number',
  'For information US call:', '001-800-227-6701', 'Europe call:','+32 14 57 52 11',
  'Emergency Number US:', '001-201-796-7100', 'Europe', '+32 14 57 52 99', 'chemtrec tel. no. US',
  '001-800-424-9300', '001-703-527-3887', 'Lane',  'Fair', 'Lawn', 'NJ 07410', '(201) 796-7100',
  'Acros Organics', 'Skin Sensitization', 'Category 1', 'Category 2', 'Category 3', 'Category 4',
  'Category 0', 'Thermo Chemicals', 'For more information', 'For information US', 'call:',
  'Emergency', 'Number', 'US:', 'CHEMTREC', 'Tel No.', 'Tel. No.',
];
